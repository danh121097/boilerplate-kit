...
describe("auth-token-storage", () => {
  beforeEach(() => {
    installLocalStorage();
    // Nuxt registers token slots via lazy resolvers (key depends on runtime app prefix).
    registerServiceToken("ADMIN", {
      access: () => "ADMIN_ACCESS_SLOT",
      refresh: () => "ADMIN_REFRESH_SLOT",
    });
  });
  ...
  it("returns null during SSR (no window / localStorage)", () => {
    vi.unstubAllGlobals(); // simulate server: window + localStorage undefined
