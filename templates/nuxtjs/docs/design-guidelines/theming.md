...
Colors and radius are CSS variables defined in `app/css/main.css`. They are
...
4. The base surface follows tokens because components reference `bg-background` /
   `text-foreground` token utilities; `main.scss` holds non-color globals
   (safe-area vars, box-sizing reset) but does not bind body colors.

## Token Reference

HSL values, as defined in `main.css`. `--radius` is `0.625rem`.
...
The base palette is intentionally **neutral grayscale**; `chart-*` carries the
only accent hues.
