...
describe("auth-token-storage", () => {
  ...
  it("returns null on server (localStorage undefined) — SSR guard (getAccessToken)", () => {
    vi.stubGlobal("localStorage", undefined);
    expect(getAccessToken("MAIN")).toBeNull();
  });

  it("returns null on server (localStorage undefined) — SSR guard (getRefreshToken)", () => {
    vi.stubGlobal("localStorage", undefined);
    expect(getRefreshToken("MAIN")).toBeNull();
  });

  it("persistAccessToken is a no-op on server — SSR guard", () => {
    vi.stubGlobal("localStorage", undefined);
  ...
  it("persistRefreshToken is a no-op on server — SSR guard", () => {
    vi.stubGlobal("localStorage", undefined);
  ...
  it("clearServiceTokens is a no-op on server — SSR guard", () => {
    vi.stubGlobal("localStorage", undefined);
    expect(() => clearServiceTokens("MAIN")).not.toThrow();
  });

  it("clearAuthTokens is a no-op on server — SSR guard", () => {
    vi.stubGlobal("localStorage", undefined);
