...
describe("AuthModel", () => {
  ...
  it("login does not persist refresh token when backend omits it", async () => {
    const noRefresh = { ...RESULT, tokens: { accessToken: "AT" } };
    vi.spyOn(AuthModel.api, "post").mockResolvedValue({ success: true, data: noRefresh } as never);
