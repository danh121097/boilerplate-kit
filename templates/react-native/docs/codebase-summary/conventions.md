...
- `.ts` / `.tsx` files: kebab-case (e.g. `auth.ts`, `form-field.tsx`).
- Hooks: `useXxx.ts` camelCase (exception to kebab-case — matches function name).
- Screens: PascalCase display name inside the file; kebab-case filename (e.g. `login.tsx` exports `LoginScreen`).
- UI primitives: PascalCase display name; kebab-case filename (e.g. `button.tsx` exports `Button`).
