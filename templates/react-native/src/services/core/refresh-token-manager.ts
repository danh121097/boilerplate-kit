 ...
 *
 * SecureStore persist/clear are async, so the success/failure branches `await`
 * storage. `inFlight` is assigned synchronously before the first await, so the
 * single-flight dedup is preserved.
 */
export class RefreshTokenManager {
  ...
  getFreshToken(): Promise<string> {
    ...
    this.inFlight = this.refresh()
      .then(async (token) => {
        await persistAccessToken(token, this.service);
        return token;
      })
      .catch(async (error) => {
