...
```
feat(auth): add refresh-token reuse detection on /refresh
fix(token): pin RS256 for access and reject refresh tokens at access verify
refactor(guard): fold hmac/jwt/role into one composite SecurityGuard
test(auth): cover login failure for inactive users
docs(code-standards): document AppException error contract
