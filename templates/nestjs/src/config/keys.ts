 ...
 *
 * Priority:
 *   1. Read PEM files from privPath / pubPath (+ self-test).
 ...
 */
export function loadRsaKeyPair(
  privPath: string,
  pubPath: string,
  nodeEnv: string,
  ...
  if (!EPHEMERAL_ENVS.has(nodeEnv)) {
    throw new Error(
      "[keys] RSA key files missing — set JWT_PRIVATE_KEY_PATH / JWT_PUBLIC_KEY_PATH (run `pnpm keys`)",
    );
  }
