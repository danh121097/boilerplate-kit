...
/**
 * Reusable pagination helpers — ported from express utils/pagination.ts verbatim.
 *
 * Two strategies:
 *   - OFFSET (page/limit): jump-to-page with total count; good for admin lists.
 *   - CURSOR (keyset on _id): stable under inserts, fast at scale; good for feeds.
 *
 * Pure + model-agnostic: no DB calls here. Parsers clamp + default so junk
 * page/limit degrades to safe values instead of erroring.
 */
...
/** Query bag as Nest delivers it — values arrive as strings (or arrays). */
...
export function parseCursorPagination(query: Query): CursorParams {
  ...
  const cursor =
    typeof raw === "string" && Types.ObjectId.isValid(raw) ? raw : undefined;
