...
describe("JWT Utils", () => {
  ...
  describe("access token forgery is rejected (alg pinned to RS256)", () => {
    it("rejects an HS256 token signed with the public key as secret", () => {
      const forged = jwt.sign({ ...payload, token_use: "access" }, config.jwtAccessPublicKey, {
        algorithm: "HS256",
      });
    ...
    it("rejects an HS256 token signed with an arbitrary secret", () => {
      const forged = jwt.sign({ ...payload, token_use: "access" }, "attacker-chosen-secret", {
        algorithm: "HS256",
      });
      expect(() => verifyAccessToken(forged)).toThrow();
    });
    ...
    it("rejects an alg:none token", () => {
      ...
      const body = Buffer.from(JSON.stringify({ ...payload, token_use: "access" })).toString(
        "base64url",
      );
    ...
    it("rejects an RS256 token signed by an unrelated keypair", () => {
      ...
      const forged = jwt.sign({ ...payload, token_use: "access" }, privateKey, {
        algorithm: "RS256",
      });
      expect(() => verifyAccessToken(forged)).toThrow();
    });
