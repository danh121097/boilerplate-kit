...
describe("verifyHmac", () => {
  ...
  it("rejects a missing signature", () => {
    expect(verifyHmac({ ...base, ctime: Date.now(), sig: "" })).toBe("missing signature");
  });

  it("rejects a non-numeric timestamp", () => {
    expect(verifyHmac({ ...base, ctime: "nope", sig: validSig(Date.now()) })).toBe(
      "invalid timestamp",
    );
  });
  ...
  it("rejects an expired timestamp", () => {
    ...
    expect(verifyHmac({ ...base, ctime: old, sig: validSig(old) })).toBe("timestamp expired");
  });

  it("rejects a wrong-length signature", () => {
    const ctime = Date.now();
    expect(verifyHmac({ ...base, ctime, sig: "AAAA" })).toBe("invalid signature format");
  });
