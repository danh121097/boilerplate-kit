...
Pinia stores are named `useXStore` and live in `src/stores/<kebab-name>.ts`.
Always import them explicitly (they are **not** auto-imported).

```ts
// store: src/stores/socket-io.ts  ->  export const useSocketIOStore = ...
