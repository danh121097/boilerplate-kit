 ...
 * env so a deployment can change them without code edits. `VITE_APP_ENDPOINT` is
 * the origin (also used bare by the Socket.IO client); `VITE_API_PREFIX` defaults
 * to `/api/v1` (mirrors the express `API_PREFIX`).
 */
